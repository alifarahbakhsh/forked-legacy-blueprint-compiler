...
func (n *RedisNode) Accept(v Visitor) {
	v.VisitRedisNode(v, n)
}

func (n *RedisNode) GetNodes(nodeType string) []Node {
	var nodes []Node
...
func GenerateRedisNode(node parser.DetailNode) Node {
	...
	return &RedisNode{Name: node.Name, TypeName: "RedisCache", Params: params, ClientModifiers: cmodifiers, ServerModifiers: smodifiers, DepInfo: deploy.NewDeployInfo()}
}

func (n *RedisNode) getConstructorBody(info *parser.ImplInfo) string {
	body := ""
...
func (n *RedisNode) GenerateClientNode(info *parser.ImplInfo) {
	...
	client_node := &ServiceImplInfo{Name: n.Name, ReceiverName: "c", Methods: methods, Constructors: []parser.FuncInfo{constructor}, Imports: imports, Fields: fields, MethodBodies: bodies, PluginName: "Redis"}
