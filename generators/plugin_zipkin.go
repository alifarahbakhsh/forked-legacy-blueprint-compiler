...
func (n *ZipkinNode) Accept(v Visitor) {
	v.VisitZipkinNode(v, n)
}

func (n *ZipkinNode) GetNodes(nodeType string) []Node {
	var nodes []Node
...
func GenerateZipkinNode(node parser.DetailNode) Node {
	...
	return &ZipkinNode{Name: node.Name, TypeName: "ZipkinTracer", Params: params, ClientModifiers: cmodifiers, ServerModifiers: smodifiers, DepInfo: deploy.NewDeployInfo()}
}

func (n *ZipkinNode) getConstructorBody(info *parser.ImplInfo) string {
	body := ""
...
func (n *ZipkinNode) GenerateClientNode(info *parser.ImplInfo) {
	...
	client_node := &ServiceImplInfo{Name: n.Name, ReceiverName: "t", Methods: methods, Constructors: []parser.FuncInfo{constructor}, Imports: imports, Fields: fields, MethodBodies: bodies, PluginName: "Zipkin"}
